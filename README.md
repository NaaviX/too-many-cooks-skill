...
| **Claude Code** (plugin) | `dist/claude-code-plugin/` (bundles skill + `/toomanycooks-arbitrage`, `/toomanycooks-rates`, `/toomanycooks-setup` + MCP) — install via marketplace (below) | prompts for the key at enable time (`userConfig`) |
| **Agent Skills** (`npx skills add`) | `npx skills add github.com/naavix/too-many-cooks-skill` (Cursor/ChatGPT/Codex/Claude) | repo-root `mcp-snippet.json` → host MCP config |
...
Any [Agent-Skills](https://github.com/vercel-labs/agent-skills) host pulls the
skill straight from this repo with no clone:

```bash
npx skills add https://github.com/naavix/too-many-cooks-skill
```

It installs the know-how only — add the MCP block (below) once afterwards. The
CLI reads `skills/toomanycooks/SKILL.md` + `mcp-snippet.json`, committed at the
repo root (generated by `npm run build` from `canonical/`). Building one of the
other platforms from source? `./install.sh <platform>` copies the right file into
place and prints the MCP snippet (`./install.sh --list` for the supported ones).
...
Run **`/toomanycooks-setup`** (Claude Code plugin) once to pick your default exchanges,
